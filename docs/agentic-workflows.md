...
| Topic                                                                        | What to know                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **The approval gate (biggest gotcha)**                                       | By default GitHub holds _every_ workflow on a coding-agent PR for manual approval (`action_required`). Turn it off at repo **Settings → Copilot → Cloud agent → "Require approval for workflow runs" = OFF** (keep **"Allow automations" = ON**) — repo-admin only, and separate from the fork / Actions-General settings.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **Delegating to the coding agent**                                           | Delegate by **assigning the issue/PR to `copilot`** — that's what the scout does via `create-issue { assignees: [copilot] }`. (In the PR/timeline context the agent then surfaces as the author `copilot-swe-agent`; match on that when scoping, but don't hard-code its node id — resolve the assignable actor at runtime via GraphQL `suggestedActors(capabilities:[CAN_BE_ASSIGNED])` / `replaceActorsForAssignable` when assigning by hand.)                                                                                                                                                                                                                                                                                                                                                                                             |
| **A user PAT is needed only for delegation + un-draft (not the CLI engine)** | Two separate concerns. **(1) Running the Copilot CLI engine:** since the [Jul 2026 changelog](https://github.blog/changelog/2026-07-02-copilot-cli-no-longer-needs-a-personal-access-token-in-github-actions/) this uses the built-in `GITHUB_TOKEN` plus a `copilot-requests: write` permission (AI credits bill to the org). The scout's compiled `.lock.yml` already does exactly this (`COPILOT_GITHUB_TOKEN: ${{ github.token }}`, `GH_AW_LLM_PROVIDER: github`), so **no PAT is required to run the CLI**. **(2) Two things `GITHUB_TOKEN` still can't do:** trigger the Copilot _coding agent_ (assigning an issue to `copilot`) and `markPullRequestReadyForReview` ("Resource not accessible by integration"). For those, store a user PAT as the secret **`GH_AW_AGENT_TOKEN`** and use it for delegation _and_ for `gh pr ready`. |
...
- [ ] **Secret `GH_AW_AGENT_TOKEN`** — a user PAT (Copilot-licensed) that can delegate to the coding
      agent and un-draft PRs. Prefer a **fine-grained PAT** scoped to this repo with only the
      permissions it needs (Contents, Issues, Pull requests: read & write); a classic PAT with `repo`
      scope works as a fallback. **Note:** this PAT is _not_ used to run the Copilot CLI engine — that
      authenticates with the built-in `GITHUB_TOKEN` (see the next item). It's only for delegating to
      the coding agent and un-drafting PRs.
- [ ] **Copilot CLI billing policy** — enable **"Allow use of Copilot CLI billed to the
      organization"** (org Settings → Copilot; on by default when the "Copilot CLI" policy is enabled).
      With it on, the scout's CLI engine authenticates via the built-in `GITHUB_TOKEN` and the
      `copilot-requests: write` permission — no secret required for the CLI itself. AI credits consumed
      by the scout bill to the org.
...
| Scout runs but **creates no issues**                 | `deduplicate-by-title` matched an existing (even closed) issue, or nothing in the 14-day window was demoable (a `[aw] No-Op Runs` issue is filed). | Expected steady-state; seed a demo manually by creating an issue and assigning it to `copilot` if you need to test downstream.                 |
...
4. **Give the workflow the right token — and know which token does what.** Running the Copilot CLI
   engine now uses the built-in `GITHUB_TOKEN` (+ a `copilot-requests: write` permission); a user PAT
   is only needed for what `GITHUB_TOKEN` still can't do — trigger the _coding agent_ and un-draft a PR.

...
difficulty: Core
icon: shield-check
summary: CodeQL, secret scanning, dependency review, and Autofix in the workflow.
---
...
<Aside type="tip" title="Why this works">
  CodeQL's default setup auto-detects the JavaScript/TypeScript codebase and runs semantic analysis
  that catches issues like SQL injection, XSS, and prototype pollution — beyond what linters find.
</Aside>
...
<Aside type="tip" title="Why this works">
  Copilot Autofix understands the vulnerability context and generates a targeted fix — not just a
  generic suggestion. It accounts for the surrounding code and project patterns.
</Aside>
...
<Aside type="tip" title="Why this works">
  Dependency review runs automatically on PRs that change manifests, catching known vulnerabilities
  and license issues before they reach the main branch.
</Aside>
...
<Aside type="caution">
  Push protection actively blocks commits containing detected secret patterns. Use a clearly
  fake/test secret for the demo — never push real credentials.
</Aside>
...
<Aside type="tip" title="Why this works">
  The inline PR experience means developers see and fix security issues in their normal review
  workflow — no context switching to a separate security dashboard.
</Aside>

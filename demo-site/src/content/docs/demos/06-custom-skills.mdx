...
icon: sparkles
summary: Generate real artifacts — diagrams and architecture visuals from a prompt.
---
...
<Aside type="tip" title="Why this works">
  The skill translates natural language into Excalidraw's JSON format, creating properly positioned
  elements with labeled arrows showing communication patterns. The output can be opened directly in
  Excalidraw.
</Aside>
...
<Aside type="tip" title="Why this works">
  The skill queries your Azure subscription, discovers all resources in the resource group,
  identifies relationships (e.g., AKS → ACR, PostgreSQL → VNet), and renders a Mermaid diagram.
</Aside>
...
<Aside type="caution">
  This skill requires Azure CLI authentication (`az login`) and appropriate read permissions on the
